...
create unique index if not exists rh_member_documents_one_profile
  on public.rh_member_documents(member_id) where category='profile';
...
create trigger rh_member_document_limit before insert on public.rh_member_documents
...
begin
  foreach t in array array[
    'rh_businesses','rh_staff','rh_permissions','rh_settings','rh_groups','rh_members','rh_guarantors','rh_member_documents',
...
on public.rh_member_documents for all to authenticated
using (business_id=public.rh_current_business_id())
with check (
  business_id=public.rh_current_business_id()
  and exists (
